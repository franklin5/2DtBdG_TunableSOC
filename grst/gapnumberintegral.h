...
private:
  ...
  double _Delta, _mu, _Sz0;
  Matrix4d _bdg;
 public:
  cSeek_Gap_Number(const double Delta, const double Mu, const sPara& para, const sGauss& gauss)
    : _myeps(1e-12), _Eb(para.t), _h(para.h), _v(para.v), _NK(gauss.N), _kc(gauss.kc),
    ...
    void getresult(double& delta, double& mu, double& Eg, double& Sz0){
      delta = _Delta; 
      mu = _mu; 
      Eg = abs(_h-sqrt(_mu*_mu+_Delta*_Delta)); 
      Sz0 = _Sz0;
    }
