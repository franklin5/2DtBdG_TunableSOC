...
void cSeek_Gap_Number::spintexture(){
  double kx = 0.0, xi = kx*kx-_mu;
  _bdg(0,0) = xi+_h;
  _bdg(0,1) = _v*kx;
  _bdg(0,2) = 0.0;
  _bdg(0,3) = -_Delta;
  ...
  _bdg(1,3) = 0.0;
  _bdg(2,0) = 0.0;
  _bdg(2,1) = _Delta; // should have been conjugate of Delta here. Note for the tBdG update!
  _bdg(2,2) = -(xi+_h);
  _bdg(2,3) = _v*kx;
  _bdg(3,0) = -_Delta; // should have been conjugate of Delta here. Note for the tBdG update!
  _bdg(3,1) = 0.0;
  _bdg(3,2) = _v*kx;
  _bdg(3,3) = -xi+_h;
  ...
  es.compute(_bdg);
  double tmpSz0 = pow(fabs(es.eigenvectors().col(2)[1]),2.0) - pow(fabs(es.eigenvectors().col(2)[0]),2.0);
  tmpSz0 += pow(fabs(es.eigenvectors().col(3)[3]),2.0) - pow(fabs(es.eigenvectors().col(3)[2]),2.0);
  /*for(int i = 0;i<4;++i){
